...
def telegram_bot_sendtext(bot_message):
            ...
            cur.execute("SHOW COLUMNS FROM electrodomesticos_details")
            b=cur.fetchall()
...
def telegram_bot_sendtext_hogar(bot_message):
            bot_token  = '5213011783:AAFrz4Zxl04baZyFVt9CXHp6yAackLSaQRQ'
            bot_chatID = '938945400'
            idGrupo = '-1001603791163'
            enviar_text = 'https://api.telegram.org/bot' + bot_token + '/sendMessage?chat_id=' + idGrupo + '&parse_mode=Markdown&text=' + bot_message
            response = requests.get(enviar_text)
            #creating arrays
            column_name = []
...
query="select urls,status, oferta from electrodomesticos_details"
...
for row in a:
    ...
    if soup.find('span',class_="price-value" )  != None and soup.find('p',class_="lp-badge promotion-badge lp-font--barlow-medium" ) != None:
     ...
     #if soup.find('div',class_="amasty-label-text" )  != None:
         ...
         if ofertaurlcompara.strip() != ofertabd:
                if porcentaje != None:
                    ...
                    if(int(porcentaje.strip()) >= 60 and int(porcentaje.strip()) < 100):
                        ...
                        print ('UPDATE electrodomesticos_details SET oferta="%s", titulo="%s", porcentaje="%s", fecha_actualizacion="%s"  where urls="%s" '%(ofertaurlcompara.strip(),titulolimpio.strip(),porcentaje.strip(), fecha_actualizacion,link))
                        cur.execute('UPDATE electrodomesticos_details SET oferta="%s", titulo="%s", porcentaje="%s", fecha_actualizacion="%s"  where urls="%s" '%(ofertaurlcompara.strip(),titulolimpio.strip(),porcentaje.strip(), fecha_actualizacion,link))
                        db.commit()
