...
def telegram_bot_sendtext(bot_message):
            ...
            cur.execute("SHOW COLUMNS FROM zapatos_details")
            b=cur.fetchall()
     ...
     if "Agregar" in soup.find('button',class_="add-to-cart lp-button lp-button--medium lp-button--inverted lp-button--borderless lp-button--no-hover lp-font--uppercase lp-font--barlow ms-full-width" ).text:
         ...
         if ofertaurlcompara.strip() != ofertabd:
                if porcentaje != None:
                    ...
                    #print(tarjeta_final)
                    #print(" \n ")
                    if(int(porcentaje.strip()) >= 60 and int(porcentaje.strip()) < 99):
                        #print(porcentaje)
                        ...
                        print ('UPDATE zapatos_details SET oferta="%s", titulo="%s", porcentaje="%s", fecha_actualizacion="%s"  where urls="%s" '%(ofertaurlcompara.strip(),titulolimpio.strip(),porcentaje.strip(), fecha_actualizacion,link))
                        cur.execute('UPDATE zapatos_details SET oferta="%s", titulo="%s", porcentaje="%s", fecha_actualizacion="%s"  where urls="%s" '%(ofertaurlcompara.strip(),titulolimpio.strip(),porcentaje.strip(), fecha_actualizacion,link))
                        db.commit()
