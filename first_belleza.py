...
#se especifica catagoria de television
 url = "https://www.lapolar.cl/belleza/?start=%s&sz=36"%pagina
 #print(url)
 ...
 while x <= 1:
  ...
  for a in links:
   ...
   if "https://www.lapolar.cl" in a.get('value'):
    ...
    #print(url_final)
    query="select count(urls) from belleza_details where urls='%s'"%all_links
    ...
    for row in a:
     ...
     if int(link) == 0:
      #print (link)
      query= "insert into belleza_details(urls) values('%s')"%(all_links)
      #print(query)
      cur.execute(query)
      db.commit()
